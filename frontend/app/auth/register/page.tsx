...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      console.log("REGISTER RESPONSE FROM BACKEND:", payload);

      if (!res.ok) {
        setError(payload?.message || "Server returned an error.");
        return;
      }

      // ✅ backend returns userId at top level
      const userId =
        payload.userId || payload.id || payload._id || payload?.data?.userId;

      if (!userId) {
        setError(
          "Backend did not return user id (register). Check console for raw payload."
        );
        return;
      }
